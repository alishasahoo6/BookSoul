...
def request_with_retry(method, url, max_retries=3, initial_backoff=1.0, **kwargs):
    ...
    for attempt in range(max_retries):
        try:
            ...
            # If rate limited or server overloaded, retry with backoff
            if response.status_code in (429, 503):
                if attempt < max_retries - 1:
                    logger.warning(
                        f"HTTP {response.status_code} received. Retrying in {backoff:.1f}s... (Attempt {attempt+1}/{max_retries})"
                    )
                    time.sleep(backoff)
                    backoff *= 2
                    continue
            ...
            if attempt < max_retries - 1:
                logger.warning(
                    f"Request failed with {type(e).__name__}. Retrying in {backoff:.1f}s... (Attempt {attempt+1}/{max_retries})"
                )
                time.sleep(backoff)
                backoff *= 2
            else:
